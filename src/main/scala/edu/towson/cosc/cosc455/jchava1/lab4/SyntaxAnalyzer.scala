...
class SyntaxAnalyzer {
  ...
  val ADV: List[String] = List("accidently", "quickly", "secretly")
  ...
  //Method implements the BNF rule for an adjective
  def Adj() = {
  ...
  //Method implements the BNF rule for an adverb
  def Adv() = {
    if (ADV contains Compiler.currentToken) {
      Compiler.Scanner.getNextToken()
    }
  }
}
